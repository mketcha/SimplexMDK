# 1/30/2015
# Model: Dirichlet Generalized SBM 
...
for (run in 1:4) {
  ...
  for (i in 1:trials) {
    if (i%%100 ==0)
      print(i)
    d_graph <- generateDirichletSBM_BASE( BASE, PI, n, r);
    P <- d_graph$P
    P_Bar <- matrix(0,n,n);
    for (h in 1:m) {
      g <- P2Adj(P);
...
plot(d_graph$xDir[,1],d_graph$xDir[,2])
title("Dirichlet Latent Space")
