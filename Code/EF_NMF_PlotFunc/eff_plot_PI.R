...
Bs = list(B2_1,B2_1,B2_1,B2_1,B2_1)
PI4 <- c( .25, .25, .25, .25);
PI3 <- c(.33, .33,.34)
PI2_1 <- c(.5,.5);
PI2_2 <- c(.25,.75);
...
PI2_4 <- c(.05,.95);
PI2_5 <- c(.01,.99);
PIs = list(PI2_1,PI2_2,PI2_3,PI2_4,PI2_5)
trials <- 1000;
max_k <- 4;
nclust <- 3;
...
var_M = matrix(0,5,7);
eff_N = matrix(0,5,7);
eff_M_S = matrix(0,5,7);
eff_N_S = matrix(0,5,7);
eff_M_var = matrix(0,5,7);
eff_N_var = matrix(0,5,7);
bias_M_Naive = matrix(0,5,7);
bias_M_ASE = matrix(0,5,7);
err_byBlock = list(matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7))
err_Naive_byBlock = list(matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7))
eff_byBlock = list(matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7))
var_byBlock = list(matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7),matrix(0,3,7))
for (ind in 3:3){
  for (run in 1:7) {
    print(c(ind, run))
    PI = PIs[[ind]];
    ...
    for (i in 1:trials) {
      ...
      error_Phat_noClust[i,1] <- mean(err_v_noClust^2);
      bias_ASE[i,1] = mean(err_v_noClust);
      eff_M_bySample[i,1] <- mean(err_v_noClust^2)/error_mean[i,1];
      err_Assign = P_Assign[upper.tri(P_Assign)];
      for (k in 1:3) {
        err_block[i, k] = mean((err_v_noClust[(err_Assign==k)])^2);
        err_Naive_block[i,k] = mean((err_v[(err_Assign==k)])^2);
        eff_block[i, k] = mean((err_v_noClust[(err_Assign==k)])^2)/mean((err_v[(err_Assign==k)])^2)
      }
    ...
    bias_M_ASE[ind,run] = mean(bias_ASE);
    bias_M_Naive[ind,run] = mean(bias_Naive);
    eff_M_S[ind,run] = mean(eff_M_bySample[,1])
    eff_M_var[ind,run] = var(eff_M_bySample[,1])
    eff_M[ind,run] = mean(error_Phat_noClust[,1])/mean(error_mean)
    for (k in 1:3){
      err_byBlock[[ind]][k,run] = mean(err_block[,k],na.rm = TRUE)
      err_Naive_byBlock[[ind]][k,run] = mean(err_block[,k],na.rm = TRUE)
      eff_byBlock[[ind]][k,run] = mean(eff_block[,k],na.rm = TRUE)
      var_byBlock[[ind]][k,run] = var(eff_block[,k],na.rm = TRUE)
    }
  }
}
...
inv2N = (2/(MN_M[,2]))
matplot( N_mat,cbind(inv2N,t(eff_byBlock[[3]])),type = "b", xlab= "N", ylab = "RE", pch = 19, lwd = 2,col = c("black", "red", "green", "blue", "orange", "cyan"))
title("Relative Efficiency with M =100")
legend(locator(1), , legend = c("2/N",  "p = .42", "p = .2", "p = .7"), lty = c(19,19,19,19), col = c("black", "red", "green", "blue", "orange", "cyan"), lwd = 2)

matplot( N_mat[,1:3],t(eff_byBlock[[3]])/matrix(rep(inv2N,3), ncol =3),type = "b", xlab= "N", ylab = "RE*N/2", pch = 19, lwd = 2, col = c("red", "green", "blue", "orange", "cyan","purple"))
