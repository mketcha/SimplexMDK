...
#B2_2 <- matrix(c(.042, .02, .02, .07),2)*3;
#B2_3 <- matrix(c(.02, .005, .005, .03),2);
B3_1 <- matrix(c(.42, .2, .3, .2,.5, .15, .3, .15, .7),3);
#B3_2 <- matrix(c(.042, .02, .03, .02,.05, .015, .03, .015, .07),3)*3;
#B3 <- matrix(c(.42, .23, .35, .23,.5, .15, .35, .15, .7),3);
B4_1 <- matrix(c(.42, .2, .3 ,.05, .2, .49, .35, .15, .3, .35, .6, .25, .05 , .15,.25, .7),4);
#B4_2 <- matrix(c(.042, .02, .03 ,.005, .02, .049, .035, .015, .03, .035, .06, .025, .005 , .015,.025, .07),4)*3;
#Bs = list(B2_1,B2_2,B3_1,B3_2,B4_1,B4_2)
...
eff_M_NMF = matrix(0,3,6);
eff_M = matrix(0,3,6);
eff_M_S_NMF = matrix(0,3,6);
eff_M_S = matrix(0,3,6);
eff_M_var_NMF = matrix(0,3,6);
eff_M_var = matrix(0,3,6);
err_M = matrix(0,3,6)
err_M_NMF = matrix(0,3,6)
for (ind in 1:3){
  for (run in 1:6) {
    print(c(ind, run))
    PI = PIs[[ind]];
...
# inv2N = (2/(MN_M[,2]))
# matplot( N_mat[2:4,],cbind(t(eff_M[,2:4]), t(eff_M_NMF[,2:4])),type = "b", lty = c(1,1,1,2,2,2),  xlab= "N", ylab = "RE", pch = 19, lwd = 2,col = c("red", "green", "blue"))
# title("Relative Efficiency with M =100, k = 3")
# legend(locator(1), c("ASE k, d = 2", "ASE k, d = 3", " ASE k, d = 4", "NMF k, d = 2", "NMF k, d = 3", " NMF k, d = 4"), lty = c(1,1,1,2,2,2), col = c("black", "red", "green", "blue"), lwd = 2)
# 
# par(mfrow=c(1,1));
# matplot( N_mat,cbind(t(eff_M[,2:4]), t(eff_M_NMF[2:4]))/matrix(rep(inv2N,6), ncol =6),type = "b", xlab= "N", ylab = "RE*N/2", pch = 19, lwd = 2, col = c("red", "green", "blue"))
