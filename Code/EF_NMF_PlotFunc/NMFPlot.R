...
Bs = list(B2_1,B2_2,B2_3,B3_1,B3_2,B4_1,B4_2)
#Bs = list(B2_1,B3_1,B4_1)
PI4 <- c( .25, .25, .25, .25);
PI3 <- c(.33, .33,.34)
PI2 <- c(.5,.5);
PIs = list(PI2,PI2,PI2,PI3,PI3, PI4,PI4)
trials <- 1000;
max_k <- 4;
nclust <- 3;
myoptions <- igraph.arpack.default
myoptions$maxiter <- 20000
eff_M_NMF = matrix(0,7,6);
eff_M = matrix(0,7,6);
eff_M_S_NMF = matrix(0,7,6);
eff_M_S = matrix(0,7,6);
eff_M_var_NMF = matrix(0,7,6);
eff_M_var = matrix(0,7,6);
err_M = matrix(0,7,6)
err_M_NMF = matrix(0,7,6)
for (ind in 1:7){
  for (run in 1:6) {
    print(c(ind, run))
    PI = PIs[[ind]];
