...
#       error_Phat[i,j] <- norm(err_v, type = "2");
  ...
  vioplot( error_mean, error_Phat_noClust[,1], error_Phat_noClust[,2], error_Phat_noClust[,3], error_Phat_noClust[,4], names = c("Naive", "d=1", "d=2", "d=3","d=4"), col =c("blue"));
  #vioplot(error_Phat_noClust[,5], at = 10, col = c("green"), add= TRUE)
  title(main=paste("M = ", m, ", N = ", n,", Exact SBM"))
  #legend(locator(1), legend= c("Naive", "X*X'", "Up to 10 clusters"), fill = c("magenta","green", "blue"))
  vioplot(error_mean, at = 1, col = c("green"), add= TRUE)
...
legend(locator(1), legend= c("Naive", "X_hat*X_hat'"), fill = c("green", "blue"))
